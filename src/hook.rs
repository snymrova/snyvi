...
pub fn run(paths: &Paths) -> Result<()> {
    ...
    let payload = Payload {
        ...
        session: event
            .get("session_id")
            .and_then(Value::as_str)
            .map(|s| format!("claude {}", &s[..s.len().min(8)])),
...
/// Merge the hook into ~/.claude/settings.json, preserving everything else in it.
pub fn install(exe: &str) -> Result<PathBuf> {
    ...
    let post = hooks
        ...
        .entry("PostToolUse")
        .or_insert(json!([]));
    let list = post.as_array_mut().context("PostToolUse is not an array")?;
