...
| First load < 60 KB gzipped UI | Shell page, 48 KB as transferred; first paint, 60 KB (`bench/bytes.mjs`) | 36 KB; 55.1 KB | Held, and a row reads the sum at last. When 1.0 shipped it was 63.5 KB against the 60 the table asked for, found by hand and enforced by nothing; `bench/bytes.mjs` made it a number a push can break, and then made the overage visible enough to pay -- the diagram driver was 11.6 KB gzipped on every page load, driving a library that had been lazy since 0.2. Moved to `ui/mmd.js`, fetched with the first diagram. |
