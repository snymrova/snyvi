...
| A way back from every page | Only a document read over a desk had a ✕; everywhere else the way out was Back, which walks through every document `j` or a link opened on the way, and a deep link has nothing behind it at all. Every document and file now has a ✕ in its head that goes to the screen the reading began on -- the Inbox, a folder, a desk -- past everything read in between, and to the Inbox when there is none. Esc does it too, once nothing else is over the page. | S | **done 1.6** |
| An aside can be closed | The card at the foot of the sidebar had no ✕, no Esc and no timeout: it stayed until the daemon restarted, taking height from the trees above it. A ✕ on it and Esc close it; the card stands as one line with an Undo for 4 s, as a removed row does, and then the next aside takes it. "Close all" closes the trail. The daemon only flags a closed aside, and tells every page. | S | **done 1.6** |
| Open in file manager | A folder could be opened in a terminal but not in Files, Finder or Explorer. `POST /api/reveal` takes the same ids as `/api/terminal` and resolves them through the same function, so the two cannot open different places; a desk's folder goes behind the desk's gate. The button sits beside Open terminal here, in a folder's menu, and on a desk's Folder line. | XS | **done 1.6** |
| No cap across desks | Every desk holds four panels and there is no longer a total of eight across them: a reader with three projects running wants three desks of panels. A panel no page is watching makes one frame a second instead of sixty -- the frame is still what files lines into its kept scrollback -- and a page that attaches is caught up at once. A narrow window no longer refuses a third panel; the ones it has no room for are tabs. | S | **done 1.6** |
| Panels that move | A panel's place was fixed at the slot it opened in. A head dragged onto another panel, or ⌃⌥⇧ and an arrow, trades their places; the number goes with the place, so ⌃⌥2 is still the panel in position 2, and the documents each panel sent are renumbered in the same transaction (`POST /api/desks/{id}/move`). | S | **done 1.6** |
| Full view | ⌃⌥Z made the focused panel fill the grid, with the sidebar, the rail and the desk's head still around it, and nothing on a panel said it could. Now ⤢ on the head, a double-click on it, ⌃⌥Z or the width button put the panel in full view: the whole window, the others as tabs, a waiting one marked `!`, kept per desk until a reload. | S | **done 1.6** |
| Links in a panel | Nothing a program printed could be clicked. With Ctrl held, an http or https link under the pointer is underlined, and a Ctrl-click opens it in the browser -- a wrapped link as one, a sentence's full stop left off; a plain click never opens anything, and no other scheme is a link. | S | **done 1.6** |
| Context menus | One folder menu, and WebKit's Back and Reload everywhere else. Every row and panel now has a menu -- a folder, a file, a project, a document, a desk, a panel's head and body, a rail document, a note, a point -- each running what the row's own controls run, with the target named at the top, key hints, and close and remove last in red. The menu key and ⇧F10 open it from the keyboard, F2 renames, and ✎ left the project and desk rows for it. | M | **done 1.6** |
| An agent ticks a note | `read_desk_notes` let an agent in a panel read its desk's list and nothing more. `tick_desk_note` marks one open line done, through the same token-and-running-pane gate, and the line shows the agent's name until the reader unticks it; an agent still cannot add, change, untick or remove a line. | XS | **done 1.6** |
...
| | reads |
|---|---|
| a delete, and the way back | `Del` on an open document leaves nothing asked -- a `confirm` would hang the probe, which is the check -- the document is out of the inbox and its sidebar row says "removed" with an Undo in it, and no toast says "Deleted"; that Undo puts the document back where it was removed from; ⌘Z does the same; the ✕ on a row leaves its Undo at the row's own place and height; resting the pointer on it stops the 4 s clock, and off it the offer ends; and a removal the reader does not undo is still gone after a reload, from the inbox and from search, because the daemon did it |
| an aside, closed | the ✕ on an aside leaves "Aside closed" and an Undo in the card's place, and the daemon has it flagged; Undo puts the same aside back, unflagged; Esc on the focused card closes it without leaving the page, and the focus lands on Undo; when the 4 s run out the older aside takes the card; Close all in the trail offers one Undo for all, then the card goes; and a new aside after that still shows |
| a folder, in the file manager | a document's meta and a browsed file's both carry Open in file manager; clicking it hands the desktop's opener -- a stub that writes down its argument -- the folder the file sits in, or the daemon says there is no display |
